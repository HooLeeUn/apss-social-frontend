...
export default function GuestContentGate({ gateId, placement = "floating", className = "" }: { gateId: string; placement?: "floating" | "below" | "below-end"; className?: string }) {
  const { t } = useI18n();
  ...
  const position = placement === "below" ? "left-1/2 top-full mt-2 -translate-x-1/2" : placement === "below-end" ? "right-0 top-full mt-2" : "bottom-3 left-1/2 -translate-x-1/2";
