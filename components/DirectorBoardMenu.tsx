...
function DirectorBoardToggle({ isOpen, onClick }: DirectorBoardToggleProps) {
  return (
    <button
      ...
      className="group relative w-full max-w-[304px] rounded-2xl border border-white/20 bg-zinc-950/95 p-2 text-left shadow-[0_10px_28px_rgba(0,0,0,0.4)] transition-all duration-300 hover:border-white/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300/70"
    >
      <span
        className={`block h-5.5 w-full origin-[10%_15%] rounded-xl border border-white/20 bg-[repeating-linear-gradient(135deg,rgba(24,24,27,0.95)_0px,rgba(24,24,27,0.95)_14px,rgba(212,212,216,0.9)_14px,rgba(212,212,216,0.9)_22px)] shadow-[0_8px_16px_rgba(0,0,0,0.35)] transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          isOpen ? "-rotate-[10deg] -translate-y-[1px] translate-x-[2px]" : "rotate-0 translate-y-0 translate-x-0"
        }`}
      />
      <span className="mt-1.5 block h-6.5 rounded-lg border border-white/15 bg-zinc-900/95 px-2.5 text-center text-[0.65rem] font-medium leading-[1.65rem] tracking-[0.2em] text-zinc-300/90">
        MENU
      </span>
    </button>
  );
...
export default function DirectorBoardMenu({ isOpen, onToggle, onClose, onCloseSession }: DirectorBoardMenuProps) {
  ...
  return (
    <div ref={menuRef} className="relative w-full max-w-[304px]">
      <DirectorBoardToggle isOpen={isOpen} onClick={onToggle} />

      <div
        id="director-board-panel"
        className={`absolute left-1/2 top-full z-50 mt-2 w-[236px] max-w-[calc(100vw-2.5rem)] -translate-x-1/2 overflow-hidden rounded-2xl border border-white/15 bg-zinc-950/95 shadow-[0_12px_30px_rgba(0,0,0,0.42)] transition-all duration-300 ${
