...
function DirectorBoardToggle({ isOpen, onClick }: DirectorBoardToggleProps) {
  return (
    <button
      ...
      className="group relative w-full max-w-[372px] rounded-2xl border border-white/20 bg-zinc-950/95 px-3 py-2 text-left shadow-[0_10px_28px_rgba(0,0,0,0.4)] transition-all duration-300 hover:border-white/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300/70"
    >
      <span
      ...
      />
      <span className="mt-1.5 block h-6.5 rounded-lg border border-white/15 bg-zinc-900/95 px-3.5 text-center text-[0.65rem] font-medium leading-[1.65rem] tracking-[0.24em] text-zinc-300/90">
        MENU
      </span>
    </button>
  );
...
export default function DirectorBoardMenu({ isOpen, onToggle, onClose, onCloseSession }: DirectorBoardMenuProps) {
  ...
  return (
    <div ref={menuRef} className="relative w-full max-w-[372px]">
