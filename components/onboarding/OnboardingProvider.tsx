...
export default function OnboardingProvider() {
  ...
  const persist=useCallback(async(status:OnboardingStatus,currentStep:number|null)=>{ if(!tour||!state||!user)return; const key=onboardingQueueKey(user,tour.id,state.version); const value:PendingUpdate={status,currentStep}; try { await updateOnboardingState(tour.id,status,currentStep,state.version); localStorage.removeItem(key); } catch (error) { console.error("[Onboarding] Failed to update tour state; queued for retry.", error); localStorage.setItem(key,JSON.stringify(value)); } setState((old)=>old?{...old,status,currentStep}:old); },[state,tour,user]);
  useEffect(()=>{ let cancelled=false; const reset=window.setTimeout(()=>{ setState(null); setReady(false); setRunning(false); },0); if(!tour)return()=>clearTimeout(reset); Promise.all([getOnboardingStates(),getMyProfile()]).then(([states,profile])=>{if(cancelled)return; const identity=String(profile?.username||"authenticated"); setUser(identity); const next=states.find((item)=>item.tour===tour.id)||null; setState(next); if(next){ const key=onboardingQueueKey(identity,tour.id,next.version); const queued=localStorage.getItem(key); if(queued&&navigator.onLine){ const value=JSON.parse(queued) as PendingUpdate; updateOnboardingState(tour.id,value.status,value.currentStep,next.version).then(()=>localStorage.removeItem(key)).catch((error)=>console.error("[Onboarding] Failed to reconcile queued tour state.", error)); } } }).catch((error)=>console.error("[Onboarding] Failed to load tour state.", error)); return()=>{cancelled=true;clearTimeout(reset)}; },[tour]);
