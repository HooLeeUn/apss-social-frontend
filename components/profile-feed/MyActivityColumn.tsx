...
type CanonicalActivityVideoPage = { results?: CanonicalActivityVideo[]; next?: string | null };
...
function normalizeCanonicalActivityVideoNext(next: string | null | undefined): string | null {
  if (!next) return null;
  try {
...
async function resolveCanonicalActivityVideo({ movieId, commentId }: ActivityVideoRequest): Promise<ActivityVideoState | null> {
  let endpoint: string | null = `/movies/${encodeURIComponent(movieId)}/video-comments/`;
  while (endpoint) {
    const page = await apiFetch(endpoint) as CanonicalActivityVideoPage;
    const video = page.results?.find((item) => String(item.id) === commentId);
    if (video) {
      const likesCount = Number(video.likes_count);
      const dislikesCount = Number(video.dislikes_count);
      if (!video.video_url || !Number.isFinite(likesCount) || !Number.isFinite(dislikesCount)) return null;
      return { movieId, commentId: String(video.id), url: video.video_url, likesCount, dislikesCount, myReaction: video.my_reaction === "like" || video.my_reaction === "dislike" ? video.my_reaction : null, canDelete: video.can_delete === true };
    }
    endpoint = normalizeCanonicalActivityVideoNext(page.next);
  }
  return null;
}
...
export default function MyActivityColumn({
  ...
  const openActivityVideo = useCallback(async (request: ActivityVideoRequest) => {
    ...
    setVideoOpenError("");
    try {
      const video = await resolveCanonicalActivityVideo(request);
      if (video) setActiveVideo(video);
      else setVideoOpenError(locale === "en" ? "This video reaction is unavailable." : "Este Video Reaction no está disponible.");
    } catch (error) {
