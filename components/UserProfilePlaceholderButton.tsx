...
export default function UserProfilePlaceholderButton({
  ...
  const normalizedAvatarUrl = (() => {
    ...
    if (!avatarVersion) return avatarUrl;
    const separator = avatarUrl.includes("?") ? "&" : "?";
    return `${avatarUrl}${separator}v=${encodeURIComponent(String(avatarVersion))}`;
  })();
  ...
  return (
    ...
    >
      {normalizedAvatarUrl ? (
        ...
        <img src={normalizedAvatarUrl} alt={avatarAlt} className="h-full w-full rounded-full object-cover" />
      ) : (
        <svg
          aria-hidden="true"
          viewBox="0 0 24 24"
