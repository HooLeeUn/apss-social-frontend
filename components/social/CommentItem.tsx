...
export default function CommentItem({
  ...
  const authorIsInteractive = Boolean(onAuthorClick) && !comment.authorRestrictedCurrentUser;
    ...
    <article className={showCard ? "rounded-xl border border-white/15 bg-zinc-950/65 p-4" : "py-3"}>
      <header className="mb-2 flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          {authorIsInteractive ? (
            <button type="button" onClick={() => onAuthorClick?.(comment.authorUsername)} className="flex h-8 w-8 items-center justify-center rounded-full border border-white/25 bg-zinc-900 text-xs font-semibold text-zinc-200">
              {avatarContent}
            </button>
          ) : (
            <span className="flex h-8 w-8 items-center justify-center rounded-full border border-white/25 bg-zinc-900 text-xs font-semibold text-zinc-200">{avatarContent}</span>
          )}
          <div className="min-w-0">
            {authorIsInteractive ? (
              <button type="button" onClick={() => onAuthorClick?.(comment.authorUsername)} className="truncate text-sm font-semibold text-zinc-100 hover:text-white hover:underline">
                {comment.authorName}
              </button>
            ) : (
              <span className="block truncate text-sm font-semibold text-zinc-100">{comment.authorName}</span>
            )}
