...
const USER_SEARCH_ENDPOINTS = ["/users/", "/profile/users/", "/profile/search/"];
...
export async function searchUsersToRestrict(query: string): Promise<BlockedUser[]> {
  ...
  for (const endpoint of USER_SEARCH_ENDPOINTS) {
    try {
      const payload = await apiFetch(`${endpoint}?${new URLSearchParams({ search: trimmed }).toString()}`);
      return parseBlockedUsers(payload);
    } catch {
      continue;
    }
