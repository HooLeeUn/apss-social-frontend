...
const followedActivityCard = fs.readFileSync("components/profile-feed/SocialActivityCard.tsx", "utf8");
const followedActivityTabs = fs.readFileSync("components/profile-feed/SocialActivityTabsBlock.tsx", "utf8");
...
test("all production My Rating surfaces use the shared raising-hand icon", () => {
  ...
  assert.match(followedActivityCard, /import \{ RatingPersonRaisingHandIcon \}/);
  assert.match(followedActivityTabs, /import \{ RatingPersonRaisingHandIcon \}/);
...
test("the icon replacement preserves rating dimensions, color, and read-only ownership", () => {
  for (const source of [movieCard, favorites, followedActivityCard, followedActivityTabs]) {
    assert.match(source, /<RatingPersonRaisingHandIcon className="h-4 w-4 shrink-0 text-violet-400" \/>/);
  }
  assert.doesNotMatch(followedActivityCard, /🙋/);
  assert.doesNotMatch(followedActivityTabs, /🙋/);
