...
function MobileVideoComments({ movieId, movieTitle, moviePoster, active, notificationTarget, onNotificationTargetConsumed, logNotificationTarget, t, onAuthorClick, desktopGuest = false, trailerCompanionOpen = false }: { movieId: string; movieTitle: string; moviePoster: string | null; active: boolean; notificationTarget: { id: string; reaction: VideoCommentReaction | null } | null; onNotificationTargetConsumed: () => void; logNotificationTarget: NotificationDiagnosticLogger; t: (key: Parameters<typeof translate>[1]) => string; onAuthorClick: (username: string) => void; desktopGuest?: boolean; trailerCompanionOpen?: boolean }) {
  const router = useRouter();
  const { showGuestGate } = useGuestGate();
  const guestVideoGateId = `detail-video:${movieId}`;
  const guestTrailerHistoryTouchRef = useRef<{ startY: number; lockedScrollTop: number; gateShown: boolean } | null>(null);
  const guestTrailerRubberResetTimerRef = useRef<number | null>(null);
  ...
  const scrollHistoryCarousel = useCallback((direction: -1 | 1) => {
    const container = historyScrollRef.current;
    if (!container) return;
    if (desktopGuest && direction === 1) { showGuestGate(guestVideoGateId, "more"); return; }
  ...
  }, [desktopGuest, guestVideoGateId, showGuestGate]);
  ...
  const openExpandedVideo = useCallback((id: string) => {
    if (desktopGuest) { showGuestGate(guestVideoGateId, "expand"); return; }
  ...
  const resetGuestTrailerRubberBand = useCallback(() => {
    guestTrailerHistoryTouchRef.current = null;
    const history = historyScrollRef.current;
    if (!history) return;
    history.style.setProperty("--guest-trailer-rubber-transition", "transform 180ms ease-out");
    history.style.setProperty("--guest-trailer-rubber-y", "0px");
    if (guestTrailerRubberResetTimerRef.current !== null) window.clearTimeout(guestTrailerRubberResetTimerRef.current);
    guestTrailerRubberResetTimerRef.current = window.setTimeout(() => {
      history.style.removeProperty("--guest-trailer-rubber-transition");
      guestTrailerRubberResetTimerRef.current = null;
    }, 190);
  }, []);
  ...
  const reactionContent = <section data-mobile-video-reaction data-recording-overlay={isRecordingOverlay} data-active={active} data-video-sound-preference={soundPreference} className={`${isRecordingOverlay ? "fixed inset-0 z-50 overflow-hidden bg-black px-3 py-3" : "rounded-2xl bg-zinc-950/55 p-4"} ${active || expandedVideoId !== null ? "block" : "hidden"}`}>
    ...
    <div ref={historyScrollRef} tabIndex={desktopGuest ? 0 : undefined} onWheel={(event) => { if (!desktopGuest) return; if (trailerCompanionOpen && event.deltaY > 0) { event.preventDefault(); showGuestGate(guestVideoGateId, "more"); return; } if (event.deltaX > 0 || (event.shiftKey && event.deltaY > 0)) { event.preventDefault(); showGuestGate(guestVideoGateId, "more"); } }} onKeyDown={(event) => { if (desktopGuest && (["ArrowRight", "End", "PageDown"].includes(event.key) || (trailerCompanionOpen && event.key === "ArrowDown"))) { event.preventDefault(); showGuestGate(guestVideoGateId, "more"); } }} onScroll={(event) => { if (!desktopGuest) return; if (trailerCompanionOpen && window.matchMedia("(min-width: 1280px)").matches && event.currentTarget.scrollTop > 0) { event.currentTarget.scrollTop = 0; showGuestGate(guestVideoGateId, "more"); } if (event.currentTarget.scrollLeft > 1) { event.currentTarget.scrollLeft = 0; showGuestGate(guestVideoGateId, "more"); } }} onTouchStart={(event) => { const touch = event.touches[0]; if (!desktopGuest || !trailerCompanionOpen || !touch || window.matchMedia("(min-width: 1280px)").matches) return; if (guestTrailerRubberResetTimerRef.current !== null) window.clearTimeout(guestTrailerRubberResetTimerRef.current); event.currentTarget.style.setProperty("--guest-trailer-rubber-transition", "none"); guestTrailerHistoryTouchRef.current = { startY: touch.clientY, lockedScrollTop: event.currentTarget.scrollTop, gateShown: false }; }} onTouchMove={(event) => { const gesture = guestTrailerHistoryTouchRef.current; const currentY = event.touches[0]?.clientY; if (!gesture || currentY === undefined) return; const rawDelta = currentY - gesture.startY; if (rawDelta >= 0) return; if (event.cancelable) event.preventDefault(); event.currentTarget.style.setProperty("--guest-trailer-rubber-y", `${Math.max(-18, rawDelta / 7)}px`); if (!gesture.gateShown) { gesture.gateShown = true; showGuestGate(guestVideoGateId, "more"); } }} onTouchEnd={(event) => { const gesture = guestTrailerHistoryTouchRef.current; if (gesture) event.currentTarget.scrollTop = gesture.lockedScrollTop; resetGuestTrailerRubberBand(); }} onTouchCancel={(event) => { const gesture = guestTrailerHistoryTouchRef.current; if (gesture) event.currentTarget.scrollTop = gesture.lockedScrollTop; resetGuestTrailerRubberBand(); }} data-desktop-video-reaction-history data-can-scroll-left={canScrollHistoryLeft} data-can-scroll-right={canScrollHistoryRight} className={`desktop-dark-scrollbar ${desktopGuest ? "mt-0 xl:mt-5" : "mt-5"} space-y-3 xl:mx-auto xl:max-h-[32rem] xl:max-w-3xl xl:overflow-y-auto xl:pr-2`}>
    ...
    <GuestContentGate gateId={guestVideoGateId} portal placement="section-center" anchorRef={trailerCompanionOpen ? historyScrollRef : videoGateAnchorRef} className={trailerCompanionOpen ? "!z-[1200]" : ""} />
    </div>
...
function MovieDetailPageContent() {
  ...
  const { showGuestGate, closeGuestGate } = useGuestGate();
  const router = useRouter();
  ...
  const guestCommentsGateId = `detail-comments:${movieId}`;
  ...
  const guestTrailerCommentsTouchYRef = useRef<number | null>(null);
  ...
  }, [authHydrated, viewportHydrated, fetchMovieDetail, isDesktopGuest, locale, movieId, router]);
      ...
      <div className="mx-auto w-full max-w-[1000px] space-y-6 px-4 py-3 xl:px-8 xl:py-8">
        <div ref={stickyHeaderRef} data-mobile-detail-sticky="true" className="sticky top-0 z-40 -mx-4 space-y-6 bg-black px-4 pb-4 pt-[max(0.75rem,env(safe-area-inset-top))] xl:static xl:z-auto xl:mx-0 xl:bg-transparent xl:p-0">
          ...
          <div data-mobile-comment-tabs className="relative flex items-center justify-between gap-4 xl:hidden" role="tablist" aria-label={composerTitle}>
            {!isDesktopGuest ? <AuthenticatedProfileAvatar mobileTourTarget="detail-profile-avatar-mobile" user={authenticatedUser} label={t("movieDetailMyProfileAvatarLabel")} className="absolute left-0 top-1/2 z-10 h-9 w-9 -translate-y-1/2 cursor-pointer" /> : null}
        ...
        <div ref={videoCommentStartRef} data-video-reaction-section>
          <MobileVideoComments desktopGuest={isDesktopGuest} trailerCompanionOpen={trailerCompanionOpen} movieId={movieId} movieTitle={resolveMovieTitles(locale, movie?.titleSpanish, movie?.titleEnglish, movie?.displayTitle).primary} moviePoster={movie?.posterUrl ?? null} active={commentInputMode === "video-comment" || (trailerCompanionOpen && trailerCompanionView === "reaction")} notificationTarget={notificationTarget?.type === "video-reaction" ? { id: notificationTarget.id, reaction: notificationTarget.reaction } : null} onNotificationTargetConsumed={consumeNotificationTarget} logNotificationTarget={logNotificationTarget} t={t} onAuthorClick={handleAuthorNavigation} />
        </div>
        ...
        >
          ...
          <section data-tour-desktop="detail-public-comments" ref={publicCommentsSectionRef} data-trailer-public-comments onWheel={(event) => { if (!isDesktopGuest || !trailerCompanionOpen || trailerCompanionView !== "public-comments" || event.deltaY <= 0) return; event.preventDefault(); showGuestGate(guestCommentsGateId, "more"); }} onScroll={(event) => { if (!isDesktopGuest || !trailerCompanionOpen || trailerCompanionView !== "public-comments" || event.currentTarget.scrollTop <= 0) return; event.currentTarget.scrollTop = 0; showGuestGate(guestCommentsGateId, "more"); }} onTouchStart={(event) => { guestTrailerCommentsTouchYRef.current = event.touches[0]?.clientY ?? null; }} onTouchMove={(event) => { const previousY = guestTrailerCommentsTouchYRef.current; const currentY = event.touches[0]?.clientY; if (!isDesktopGuest || !trailerCompanionOpen || trailerCompanionView !== "public-comments" || previousY === null || currentY === undefined) return; const delta = previousY - currentY; guestTrailerCommentsTouchYRef.current = currentY; if (delta <= 0) return; if (event.cancelable) event.preventDefault(); showGuestGate(guestCommentsGateId, "more"); }} onTouchEnd={() => { guestTrailerCommentsTouchYRef.current = null; }} onTouchCancel={() => { guestTrailerCommentsTouchYRef.current = null; }} className={`space-y-3 ${shouldRenderDirectedComments && activeCommentsTab !== "public" ? "hidden xl:block" : ""}`}>
            ...
            <CommentsList
              ...
              borderlessContainer
              unboundedOnMobile={!isDesktopGuest}
            ...
            />
            {isDesktopGuest && isMobile && !trailerCompanionOpen ? <GuestContentGate gateId={guestCommentsGateId} portal placement="section-center" anchorRef={publicCommentsScrollRef} /> : null}
            </div>
            {isDesktopGuest && trailerCompanionOpen ? <GuestContentGate gateId={guestCommentsGateId} portal placement="section-center" anchorRef={publicCommentsSectionRef} className="!z-[1200]" /> : null}
          </section>
