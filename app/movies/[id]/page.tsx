...
function MobileVideoComments({ movieId, movieTitle, moviePoster, active, notificationTarget, onNotificationTargetConsumed, onTotalChange, logNotificationTarget, t, onAuthorClick, currentUserId, desktopGuest = false, trailerCompanionOpen = false, mobileViewport = false }: { movieId: string; movieTitle: string; moviePoster: string | null; active: boolean; notificationTarget: { id: string; reaction: VideoCommentReaction | null } | null; onNotificationTargetConsumed: () => void; onTotalChange: (total: number) => void; logNotificationTarget: NotificationDiagnosticLogger; t: (key: Parameters<typeof translate>[1]) => string; onAuthorClick: (username: string) => void; currentUserId?: string | number | null; desktopGuest?: boolean; trailerCompanionOpen?: boolean; mobileViewport?: boolean }) {
  const router = useRouter();
  const { showGuestGate } = useGuestGate();
  const guestVideoGateId = `detail-video:${movieId}`;
  const guestTrailerHistoryTouchRef = useRef<{ startX: number; startY: number; lockedScrollTop: number; gateShown: boolean; axis: "horizontal" | "vertical" | null } | null>(null);
    ...
    <main data-mobile-detail-layout="true" className="detail-movie-tablet-framing min-h-screen bg-black" onTouchStart={(event) => { if (document.body.classList.contains("detail-trailer-active")) handleCompanionTouchStart(event); }} onTouchMove={(event) => { if (document.body.classList.contains("detail-trailer-active")) handleCompanionTouchMove(event); }} onTouchEnd={(event) => { if (document.body.classList.contains("detail-trailer-active")) handleCompanionTouchEnd(event); }}>
      ...
      <div data-detail-movie-content className="mx-auto w-full max-w-[1000px] space-y-6 px-4 py-3 xl:px-8 xl:py-8">
        ...
        <div ref={videoCommentStartRef} data-video-reaction-section>
          <MobileVideoComments currentUserId={authenticatedUser?.id} desktopGuest={isDesktopGuest} trailerCompanionOpen={trailerCompanionOpen} mobileViewport={isMobile} movieId={movieId} movieTitle={resolveMovieTitles(locale, movie?.titleSpanish, movie?.titleEnglish, movie?.displayTitle).primary} moviePoster={movie?.posterUrl ?? null} active={commentInputMode === "video-comment" || (trailerCompanionOpen && trailerCompanionView === "reaction")} notificationTarget={notificationTarget?.type === "video-reaction" ? { id: notificationTarget.id, reaction: notificationTarget.reaction } : null} onNotificationTargetConsumed={consumeNotificationTarget} onTotalChange={setVideoReactionTotal} logNotificationTarget={logNotificationTarget} t={t} onAuthorClick={handleAuthorNavigation} />
        </div>
        ...
        >
          ...
          <section data-tour-desktop="detail-public-comments" ref={publicCommentsSectionRef} data-trailer-public-comments onWheel={(event) => { if (!isDesktopGuest || !trailerCompanionOpen || trailerCompanionView !== "public-comments" || event.deltaY <= 0) return; event.preventDefault(); showGuestGate(guestCommentsGateId, "more"); }} onScroll={(event) => { if (!isDesktopGuest || !trailerCompanionOpen || trailerCompanionView !== "public-comments" || event.currentTarget.scrollTop <= 0) return; event.currentTarget.scrollTop = 0; showGuestGate(guestCommentsGateId, "more"); }} onTouchStart={(event) => { guestTrailerCommentsTouchYRef.current = event.touches[0]?.clientY ?? null; }} onTouchMove={(event) => { const previousY = guestTrailerCommentsTouchYRef.current; const currentY = event.touches[0]?.clientY; if (!isDesktopGuest || !trailerCompanionOpen || trailerCompanionView !== "public-comments" || previousY === null || currentY === undefined) return; const delta = previousY - currentY; guestTrailerCommentsTouchYRef.current = currentY; if (delta <= 0) return; if (event.cancelable) event.preventDefault(); showGuestGate(guestCommentsGateId, "more"); }} onTouchEnd={() => { guestTrailerCommentsTouchYRef.current = null; }} onTouchCancel={() => { guestTrailerCommentsTouchYRef.current = null; }} className={`space-y-3 ${shouldRenderDirectedComments && activeCommentsTab !== "public" ? "hidden xl:block" : ""}`}>
            ...
            <CommentsList
              ...
              enablePublicModeration={!isDesktopGuest}
