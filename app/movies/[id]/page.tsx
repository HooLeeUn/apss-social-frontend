...
function MobileVideoComments({ movieId, active, notificationTarget, onNotificationTargetConsumed, logNotificationTarget, t, onAuthorClick }: { movieId: string; active: boolean; notificationTarget: { id: string; reaction: VideoCommentReaction | null } | null; onNotificationTargetConsumed: () => void; logNotificationTarget: NotificationDiagnosticLogger; t: (key: Parameters<typeof translate>[1]) => string; onAuthorClick: (username: string) => void }) {
  ...
  useEffect(() => {
    ...
    const positionTarget = async () => {
      ...
      if (desktop && container.scrollWidth > container.clientWidth) {
        ...
        // Notification targeting owns this scroll sequence. Wait for the mounted
        // card and sticky layout to settle, then perform one instant placement.
        await new Promise<void>((resolve) => requestAnimationFrame(() => requestAnimationFrame(() => resolve())));
        if (cancelled) return;
        ...
        const stickyBottom = document.querySelector<HTMLElement>('[data-mobile-detail-sticky="true"]')?.getBoundingClientRect().bottom ?? containerRectBefore.top;
        const topMargin = 8;
        const safeBottomMargin = 10;
        const visibleTop = Math.max(containerRectBefore.top, stickyBottom, 0) + topMargin;
        const visibleBottom = Math.min(containerRectBefore.bottom, window.innerHeight) - safeBottomMargin;
        const relativeVideoTop = videoRectBefore.top - containerRectBefore.top + scrollTopBefore;
        const maxScrollTop = Math.max(0, scrollContainer.scrollHeight - scrollContainer.clientHeight);
        let finalScrollTop = Math.min(maxScrollTop, Math.max(0, relativeVideoTop - (visibleTop - containerRectBefore.top)));
        const projectedBottom = containerRectBefore.top + relativeVideoTop - finalScrollTop + videoRectBefore.height;
        if (projectedBottom > visibleBottom) finalScrollTop = Math.min(maxScrollTop, finalScrollTop + projectedBottom - visibleBottom);
        ...
        const stableStickyBottom = document.querySelector<HTMLElement>('[data-mobile-detail-sticky="true"]')?.getBoundingClientRect().bottom ?? containerRectAfter.top;
        const stableVisibleTop = Math.max(containerRectAfter.top, stableStickyBottom, 0) + topMargin;
        const stableVisibleBottom = Math.min(containerRectAfter.bottom, window.innerHeight) - safeBottomMargin;
        const fullyVisible = videoRectAfter.top >= stableVisibleTop && videoRectAfter.bottom <= stableVisibleBottom;
      ...
      if (notificationTarget.reaction) {
        ...
        window.setTimeout(() => setNotificationReactionOverlay(null), reducedMotion ? 900 : 2000);
      }
  ...
  }, [navigateExpandedVideo, resetExpandedDrag]);
