...
function VideoCommentReactionButtons({ comment, disabled, className = "", t, onReact }: { comment: VideoComment; disabled: boolean; className?: string; t: (key: Parameters<typeof translate>[1]) => string; onReact: (id: string | number, reaction: VideoCommentReaction) => void }) {
  return <div className={`flex items-center gap-1 ${className}`}>
    {(["like", "dislike"] as const).map((reaction) => {
      ...
      return <button key={reaction} type="button" disabled={disabled} aria-label={label} aria-pressed={selected} title={label} className={`rounded-full px-1.5 py-1 text-[11px] font-semibold leading-none transition disabled:opacity-50 ${selected ? reaction === "like" ? "bg-emerald-500/20 text-emerald-200" : "bg-rose-500/20 text-rose-200" : "bg-black/20 text-zinc-200 hover:bg-black/40"}`} onClick={(event) => { event.stopPropagation(); onReact(comment.id, reaction); }}>
