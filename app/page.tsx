  ...
  return <>
    <AuthShell title={text.loginTitle} description={text.loginDescription} footerText={text.noAccount} footerLinkText={text.signupLink} footerHref="/signup" brandingSlot="login_logo_url" fitMobileViewport headerAction={<AuthCountrySelector />} logoAlt="QNext">
      <form onSubmit={handleLogin} noValidate className="space-y-4 sm:space-y-5">
        ...
        <div className="space-y-2"><label htmlFor="login-password" className="text-[0.82rem] font-medium uppercase tracking-[0.08em] text-zinc-200">{text.password}</label><input id="login-password" type="password" autoComplete="current-password" className={inputBaseClassName} placeholder={text.passwordPlaceholder} value={password} onChange={(e) => { setPassword(e.target.value); setFieldErrors((old) => ({ ...old, password: "" })); }} aria-invalid={Boolean(fieldErrors.password)} aria-describedby={fieldErrors.password ? "login-password-error" : undefined}/>{fieldErrors.password ? <p id="login-password-error" className="text-sm text-red-300">{fieldErrors.password}</p> : null}</div>
